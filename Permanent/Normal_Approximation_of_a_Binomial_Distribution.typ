...
Because a binomial distribution can be seen as a sum of $n$ independent
identically distributed Bernoulli(p) random variables, application of the
central limit theorem enables us to approximate a binomial distribution with a
normal distribution defined as such:
$
    T tilde.op "Binomial"(n, p) => Y tilde.dot N( n p, n p(1 - p) )
$
However, rather than a general rule about having a "sufficiently" large $n$,
when approximating a binomial distribution via a normal distribution, you must
ensure $n$ is large enough that the expected number of both successess and
failures is greater than 10. That is:
$
...
When using a normal approximation of a binomial distribution, you must account
for the fact that the binomial distribution is not continuous whereas the normal
...
over-estimating those properties of the binomial. To account for the difference,
